...
describe('Testando cadastrarProduto em produtoServices', ()=>{
  ...
  it('Deve verificar se quando a imagem é passada, é salva no servidor e excluida quando necessário', async()=>{
    const produtoMock = {
      file: {
    ...
    // Crie um buffer a partir dos dados da imagem
    const imageBuffer = Buffer.from(produtoMock.file.buffer.data);
    

    const urlImagem = await uploadImagem(produtoMock.file.originalname, imageBuffer);
    produtoMock.produto.produto_imagem = urlImagem;

    const produtoResultado = await produtosServices.cadastrarProduto(produtoMock.produto);
    expect(produtoResultado.produto.produto_imagem).toEqual(urlImagem);

    
    const {produto_imagem} = produtoResultado.produto;
    if(produto_imagem){
      const fileId = produto_imagem.split('fileId=')[1];
      const resultadoExclusaoImagem = await excluirImagem(fileId);
      
      expect(resultadoExclusaoImagem.status).toEqual(200);
    }
  })
});
