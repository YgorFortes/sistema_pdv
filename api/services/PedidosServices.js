...
class PedidosServices {
  ...
  async listarPedidos(dadosPedido){
    ...
    try {
      ...
      if(cliente_id){
        ...
        if(cliente.length <1){
          throw new ErroCustomizado('Não existe compras associadas a este cliente.',404);
        }
      }
  ...
  async criarPedido(dadosPedido){
    const {cliente_id, pedido_produtos} = dadosPedido;
    ...
    try {
      ...
      for(let pedido_produto of pedido_produtos){
        ...
        const erroProdutoEstoque = this.validaProdutoEstoque(produto, quantidade_produto);
      ...
      if(pedidoCriado){
        ...
        await this.atualizarEstoque(produtos, pedido_produtos)
      }
  ...
  async atualizarEstoque(produtos, pedido_produtos){
    for(const [index, produto] of produtos.entries()){
      ...
      await this.produtosServices.atualizarProduto(produto.id, {...produto, quantidade_estoque: quantidadeProdutoRestante});
    }
  }

  validaProdutoEstoque(produto, quantidade_produto){
    const mensagensErro = {
